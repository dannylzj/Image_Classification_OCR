...
image_file_list = get_image_file_list("./project/B384/anglebar/")
...
for key,value in dict1.items():
    ...
    grade_count = 0
    for i in value:
        # Extract dimension from image
        if (("X" in i[1][0]) or ("*" in i[1][0]) or ("×" in i[1][0]) or ("x" in i[1][0])) and (i[1][0][0].isdigit()):
    ...
    for i in value:
        ...
        if (len(i[1][0]) == 9) and (i[1][0] not in dimension):
            if (i[1][0][0].isdigit()):
                ...
                hn_count+=1
    for i in value:
        # Extract grade from image ('AH' grade only). Grades should be standard, need to know other grades
        if 'AH' in i[1][0]:
            grade.append(i[1][0])
...
df.to_csv("./project/B384/anglebar/table.csv")
print("Successfully saved output in ./project/B384/anglebar/table.csv")